...
class OptimalControlNorm(object):
    def __init__(self, time, x0, v0, xl, vl):
        # Fuel model parameters
        self.C0 = 0.245104874164936
        self.C1 = 0.00389174029574479
        ...
        self.xl = xl
        self.vl = vl
        self.vl_fun = interp1d(self.time, vl, kind='linear', assume_sorted=True)



    def F(self, U):
        X, V = self.system_solve(U)
        E = self.fuel_model(V, U) + np.norm(V - self.vl)**2
        z = self.del_t * np.sum(E)
        return z
    ...
    def f_adjoint(self, t, PQ): 
        P = PQ[0]
        P_dot = 0
        Q_dot = (self.C1 + 2 * self.C2 * self.V(t) + 3 * self.C3*self.V(t)**2 + (self.p1 + 2*self.p2*self.V(t)) * self.U(t) + (self.U(t) > 0) * self.q1 * self.U(t)**2) + 2 * (self.V(t) - self.vl_fun(t))- P
